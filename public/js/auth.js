...
export async function handleLogin(formData) {
  ...
  try {
    const result = await import('./api.js?v=20260409').then(m => m.login({ email, password }));
    if (result.success) {
...
export async function handleRegister(formData) {
  ...
  try {
    const result = await import('./api.js?v=20260409').then(m => m.register({ name, email, password }));
    if (result.success) {
      // Show success message briefly
      if (submitButton) {
        submitButton.textContent = '✓ Успешно!';
